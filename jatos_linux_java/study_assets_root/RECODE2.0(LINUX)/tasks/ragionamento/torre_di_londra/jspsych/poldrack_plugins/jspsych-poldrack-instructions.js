 ...
 *
 * Modification by Ian Eisenberg - changed navigation buttons such that the "Successiva" button reads
 * "Inizia gli esercizi" on the last page
 */
    ...
    function show_current_page() {
      if (trial.show_clickable_nav) {
        ...
        if (trial.pages.length == 1) {
          nav_html += "<button id='jspsych-instructions-next'>Inizia gli esercizi</button>"
        } else {
          if (current_page == 0) {
            nav_html += "<button id='jspsych-instructions-next'>Successiva</button>"
          } else if (current_page == trial.pages.length - 1) {
            if (trial.allow_backward) {
              nav_html += "<button id='jspsych-instructions-back'>Precedente</button>";
            }
            nav_html += "<button id='jspsych-instructions-next'>Inizia gli esercizi</button>"
          } else {
            if (trial.allow_backward) {
