(function(){
  ...
  function WordSearch(wrapEl, settings) {
    ...
    var default_settings = {
      ...
      'gridSize': 7,
      'words': ['uno', 'due', 'tre', 'quattro', 'cinque'],
	  'wordsList' : ['uno', 'due', 'tre', 'quattro', 'cinque'],
      'debug': false
    }
  ...
  WordSearch.prototype.lookup = function(selected) {
    ...
    if (this.settings.words.indexOf(words[0]) > -1 ||
      ...
      if(this.solved == this.settings.words.length){
        this.winner();
        var feed = 1;
        var Data = {timeOut: 0, feedback: feed, type: 'crucipuzzle', cresp: window.cresp};
        jatos.studySessionData = feed;
        setTimeout(() => {jatos.startComponentByPos(4,Data);}, 2000);
      }
    }
  }
  ...
  function countdown() {
    if (timeLeft == -1) {
      ...
      var timeOut = 1;
      var feed = 0;
      elem.innerHTML = '<font size="200">Il tempo è scaduto!</font>';
      var Data = {timeOut: 0, feedback: feed, type: 'crucipuzzle', cresp: window.cresp};
      jatos.studySessionData = feed;
      setTimeout(() => {jatos.startComponentByPos(4,Data);}, 2000);
    } else {
      elem.innerHTML = '<font size="5">' + timeLeft + ' secondi rimanenti</font>';
      timeLeft--;
    }
  }
   ...
   * Game Over
   */
  WordSearch.prototype.winner = function() {
    //Create overlay.
    ...
    var overlay = document.getElementById("ws-game-over-outer");
      overlay.innerHTML = "<div class='ws-game-over-inner' id='ws-game-over-inner'>"+
                            "<div class='ws-game-over' id='ws-game-over'>"+
                              "<h2>Complimenti!</h2>"+
                              "<p>Hai trovato tutte le parole!</p>"+
                            "</div>"+
                          "</div>";
  }
